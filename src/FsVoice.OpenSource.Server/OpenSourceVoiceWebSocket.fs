...
type OpenSourceVoiceWebSocketSession
    ...
    let mutable inputResampler =
        AudioPcm.StreamingLinearResampler(inputSampleRate, 24000)
    ...
    let sendAudio (phase: string) (sampleRate: int) (samples: float32 array) (cancellationToken: CancellationToken) =
        task {
            if samples.Length > 0 then
                ...
                writer.Write(
                    if String.Equals(phase, "final", StringComparison.OrdinalIgnoreCase) then
                        1uy
                    else
                        0uy
                )
    ...
    let emitVadEvent event =
        ...
        sendJson payload CancellationToken.None |> ignore
        ...
        task {
            ...
            | "audio.config" ->
                ...
                | true, value when value.TryGetInt32() |> fst ->
                    ...
                    if rate >= 8000 && rate <= 192000 && rate <> inputSampleRate then
                        lock inputLock (fun () ->
                            inputSampleRate <- rate
                            inputResampler <- AudioPcm.StreamingLinearResampler(inputSampleRate, 24000))
                | _ -> ()
            | "turn.cancel" ->
                coordinator.Cancel()
    ...
    let handleBinary (bytes: byte array) =
        if bytes.Length % sizeof<float32> = 0 then
            let samples = AudioPcm.float32FromLittleEndianBytes bytes
            let samples24k = lock inputLock (fun () -> inputResampler.Append samples)
        ...
        task {
            ...
            while socket.State = WebSocketState.Open
                ...
                | WebSocketMessageType.Text ->
                    try
                        do! handleText (Encoding.UTF8.GetString(message.ToArray())) cancellationToken
                    with ex ->
                        do!
                            sendJson
                                (box
    ...
    let acceptAsync
        ...
        task {
            if not ctx.WebSockets.IsWebSocketRequest then
                ...
                use connection =
                    new OpenSourceVoiceWebSocketSession(agent, vad, options, session, socket, logger)
