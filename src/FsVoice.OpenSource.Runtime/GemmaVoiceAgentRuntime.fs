...
type GemmaSttRuntime(gemma: IGemmaRuntime, maxTokens: int, maxAudioSeconds: float) =
    let prompt =
...
type GemmaVoiceAgentRuntime
    ...
    let gemmaRuntimeKind =
        if String.IsNullOrWhiteSpace options.Gemma.Runtime then "raw-onnx"
    ...
    let ownedGemma =
        ...
        | None ->
            ...
            | other ->
                invalidArg (nameof options.Gemma.Runtime) $"Unsupported Gemma runtime '{other}'. Use raw-onnx or ort-genai."
    ...
    let toolCatalog =
        OpenSourceTooling.create contextProviders runtimeStatusJson ignore
    ...
    let reasoningSystemPrompt (session: VoiceSession) =
        ...
        basePrompt
        + "\n\nYou own transcription-aware reasoning, tool calls, filler text, and final answer text. The service will speak your filler and final answer through Chatterbox TTS. Keep spoken text concise, natural, and easy to synthesize. Use tools when they are useful."
    ...
    let synthesize
        ...
        task {
            ...
            if not status.Ready then
                ...
                try
                    let voiceSample =
                        if String.IsNullOrWhiteSpace status.VoiceSamplePath then None else Some status.VoiceSamplePath
                        ...
                        tts.SynthesizeAsync(
                            { Phase = phase
                              ...
                              VoiceSampleTranscript =
                                if String.IsNullOrWhiteSpace options.Tts.VoiceSampleTranscript then None
                                else Some options.Tts.VoiceSampleTranscript },
    ...
    let normalizeMode mode =
        if String.IsNullOrWhiteSpace mode then
            ...
            | "gemma-chatterbox" | "gemma_chatterbox" | "open-source" | "opensource" | "gemma-tts" -> "gemma-chatterbox"
    ...
    interface IVoiceAgentRuntime with
        ...
        member _.RunTurnAsync(request, emit, cancellationToken) =
            task {
                ...
                | None ->
                    return invalidArg "sessionId" "Open-source voice agent session was not found."
                | Some session ->
                    let requestId =
                        request.RequestId
                    ...
                    File.WriteAllBytes(Path.Combine(turnDir, "user_audio_24k.f32"), AudioPcm.float32ToLittleEndianBytes request.UserAudio24k)
                    ...
                    try
                        ...
                        let! finalTts =
                            synthesize session requestId turnIndex turnDir "final" "audio.wav" finalText emit cancellationToken

                        let audioPath = finalTts |> Option.bind _.OutputPath |> Option.filter File.Exists
                        let audioUrl =
                            audioPath
                            |> Option.map (fun _ -> $"/api/open-source/sessions/{session.Id}/turns/{turnIndex}/audio.wav")

                        let detailsUrl = $"/api/open-source/sessions/{session.Id}/turns/{turnIndex}/details.json"
        ...
        member _.TryGetTurnArtifact(sessionId, turnIndex, fileName) =
            ...
            then
                ...
                let tryArtifact path =
                    ...
                    then
                        Some { Path = fullPath; ContentType = contentType }
                    else
                        None
