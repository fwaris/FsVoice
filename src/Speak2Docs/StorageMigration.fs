...
module StorageMigration =
    ...
    let migrateFromProduct oldProductName =
        try
            let oldRoot = Path.Combine(FileSystem.AppDataDirectory, oldProductName)
        ...
        with ex ->
            Console.Error.WriteLine($"Speak2Docs storage migration skipped: {ex.Message}")

    let migrateFromLegacyProduct () =
        migrateFromProduct C.PREVIOUS_PRODUCT_NAME
        migrateFromProduct C.LEGACY_PRODUCT_NAME
